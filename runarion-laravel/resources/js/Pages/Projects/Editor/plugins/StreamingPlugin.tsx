    ...
    useEffect(() => {
        ...
        if (isStreaming && streamingText && streamingText !== lastStreamingTextRef.current) {
            ...
            if (baseContentPlainText) {
                if (!baseContentPlainText.endsWith('\n') && !baseContentPlainText.endsWith(' ') &&
                    !streamingText.startsWith('\n') && !streamingText.startsWith(' ')) {
                    separator = ' ';
                }
            }

            const fullContent = baseContentPlainText + separator + streamingText;
            ...
            if (!baseContentRenderedRef.current) {
                if (effectiveBaseContent && isLexicalJSON(effectiveBaseContent)) {
                    // Load from Lexical JSON using setEditorState (outside update block)
                    try {
                        const editorState = editor.parseEditorState(effectiveBaseContent);
                        editor.setEditorState(editorState);
                        console.log('StreamingPlugin: Loaded base content from Lexical JSON');
                        ...
                        console.error('StreamingPlugin: Error loading Lexical JSON:', error);
                ...
                if (!baseContentRenderedRef.current) {
                    editor.update(() => {
                        ...
                        if (effectiveBaseContent) {
                            ...
                            const paragraphs = effectiveBaseContent.split(/\n\n+/);
                            ...
                            console.log('StreamingPlugin: Loaded base content as plain text with paragraph detection');
                        }
            ...
            const newText = streamingText.slice(renderedStreamingLengthRef.current);
            ...
            if (newText) {
                editor.update(() => {
                    ...
                    const textToProcess = renderedStreamingLengthRef.current === 0 ? separator + newText : newText;
                    ...
                    for (let i = 0; i < textToProcess.length; i++) {
                        ...
                        if (char === '\n') {
                            ...
                            if (currentText) {
                                ...
                                currentParagraphRef.current.append($createOriginTextNode(currentText, 'ai'));
                                currentText = '';
                            }
                    ...
                    renderedStreamingLengthRef.current = streamingText.length;
