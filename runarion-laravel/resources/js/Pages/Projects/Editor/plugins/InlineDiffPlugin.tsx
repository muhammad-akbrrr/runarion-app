...
function findTextInEditor(editorElement: HTMLElement, searchText: string): Range | null {
    ...
    const targetStart = matchIndex !== -1 ? matchIndex : normalizedEditorText.indexOf(normalizeForMatch(searchText.substring(0, 50)));
    ...
    while (node) {
        ...
        if (!startNode && normalizedCharCount + nodeNormalizedLength > targetStart) {
            startNode = node;
        ...
        if (startNode && normalizedCharCount + nodeNormalizedLength >= targetStart + targetLength) {
            endNode = node;
            const normalizedOffset = (targetStart + targetLength) - normalizedCharCount;
...
export function InlineDiffPlugin({ 
    ...
    const [inlinePosition, setInlinePosition] = useState<{ top: number; left: number; width: number } | null>(null);
    ...
    useEffect(() => {
        if (advisorMode === 'agent' && pendingEdits.length > 0) {
            ...
            if (activeEditId) {
                const currentEdit = pendingEdits.find(e => e.id === activeEditId);
                if (currentEdit?.status === 'stale') {
                    const nextPending = pendingEdits.find(e => e.status === 'pending');
            ...
            if (!activeEditId) {
                const firstPending = pendingEdits.find(e => e.status === 'pending');
                if (firstPending) {
            ...
            if (range) {
                ...
                if (rects.length > 0) {
                    ...
                    if (shouldScroll) {
                        ...
                        const isBelowViewport = firstRect.bottom > window.innerHeight - 200;
                        
                        if (isAboveViewport || isBelowViewport) {
                            // Find the text node's parent element and scroll to it
                            const textParent = range.startContainer.parentElement;
                            if (textParent) {
                                ...
                                setTimeout(() => {
                                    ...
                                    setTimeout(() => createHighlights(false), 400);
                                }, 50);
                            }
                        }
                    }
                ...
                if (activeEdit && !checkedForStaleness.current.has(activeEdit.id)) {
                    checkedForStaleness.current.add(activeEdit.id);
                    console.log('[InlineDiff] Text not found, marking edit as stale:', activeEdit.oldText.substring(0, 50));
                    markEditAsStale(activeEdit.id);
                    
                    // Find next pending edit that isn't stale
                    const nextPending = pendingEdits.find(e => 
                        e.id !== activeEdit.id && e.status === 'pending'
                    );
                    if (nextPending) {
    ...
    }, [activeEdit, editor, advisorMode, markEditAsStale, pendingEdits, setActiveEditId]);
    
    // Clean up highlights on unmount
    useEffect(() => {
        return () => {
            document.querySelectorAll('.inline-diff-highlight-container').forEach(el => el.remove());
            document.querySelectorAll('.inline-diff-highlight-segment').forEach(el => el.remove());
        };
    }, []);
    ...
    const handleAccept = useCallback(() => {
        ...
        if (success) {
            ...
            // Move to next
            const remaining = pendingEdits.filter(e => e.id !== activeEdit.id && e.status === 'pending');
            ...
            if (editorElement) {
                ...
                if (range) {
                    ...
                    editor.update(() => {
                        const root = $getRoot();
                        const currentContent = $convertToMarkdownString(SUPPORTED_TRANSFORMERS);
                        
                        // Try various text replacements
                        let newContent = currentContent;
                        if (currentContent.includes(activeEdit.oldText)) {
                            newContent = currentContent.replace(activeEdit.oldText, activeEdit.newText);
                        } else {
                            // Try normalized replacement
                            const normalizedOld = normalizeForMatch(activeEdit.oldText);
                            const lines = currentContent.split('\n');
                            for (let i = 0; i < lines.length; i++) {
                                if (normalizeForMatch(lines[i]).includes(normalizedOld)) {
                                    // Found the line, try to replace
                                    const lineNormalized = normalizeForMatch(lines[i]);
                                    const idx = lineNormalized.indexOf(normalizedOld);
                                    if (idx !== -1) {
                                        // Approximate replacement
                                        lines[i] = lines[i].substring(0, idx) + activeEdit.newText + 
                                                   lines[i].substring(idx + activeEdit.oldText.length);
                                        break;
                                    }
                                }
                            }
                        ...
                        if (newContent !== currentContent) {
                            ...
                            console.log('[InlineDiff] Applied edit via fallback');
                        }
                    });
    ...
    }, [activeEdit, onApplyEdit, acceptEdit, pendingEdits, setActiveEditId, editor, highlightElements]);
    ...
    }, [activeEdit, rejectEdit, pendingEdits, setActiveEditId, highlightElements]);
    ...
    }, [advisorMode, activeEdit, handleAccept, handleReject, handlePrevEdit, handleNextEdit]);
    
    // Don't render if not in agent mode, no active edit, or active edit is stale
    if (advisorMode !== 'agent' || !activeEdit || !inlinePosition || activeEdit.status === 'stale') {
        return null;
    }
    
    const pendingOnly = pendingEdits.filter(e => e.status === 'pending');
    const pendingCount = pendingOnly.length;
    const currentPendingIndex = pendingOnly.findIndex(e => e.id === activeEditId);
            ...
            <div className="bg-white rounded-lg shadow-xl border border-gray-300 overflow-hidden max-w-lg animate-in fade-in slide-in-from-top-1 duration-150">
                ...
                <div className="p-2 space-y-1.5 max-h-40 overflow-y-auto text-sm">
                    <div className="bg-red-50 rounded px-2 py-1 border-l-2 border-red-400">
                        <span className="line-through text-red-700">{activeEdit.oldText.substring(0, 150)}{activeEdit.oldText.length > 150 ? '...' : ''}</span>
                    </div>
                    <div className="bg-green-50 rounded px-2 py-1 border-l-2 border-green-400">
                        <span className="text-green-700">{activeEdit.newText.substring(0, 150)}{activeEdit.newText.length > 150 ? '...' : ''}</span>
                    </div>
                </div>
                
                {/* Keyboard hints */}
                <div className="px-3 py-1 bg-gray-50 border-t text-[10px] text-gray-400 flex gap-3 flex-wrap">
                    <span><kbd className="px-1 bg-white border rounded">⌘↵</kbd> accept</span>
                    <span><kbd className="px-1 bg-white border rounded">esc</kbd> reject</span>
                    <span><kbd className="px-1 bg-white border rounded">←</kbd><kbd className="px-1 bg-white border rounded">→</kbd> navigate</span>
                </div>
            </div>
        </div>
    );
