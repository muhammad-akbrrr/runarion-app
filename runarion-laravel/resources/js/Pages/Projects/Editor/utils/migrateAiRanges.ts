import { $getRoot, LexicalEditor, LexicalNode, ElementNode, TextNode } from 'lexical';
 ...
 */
function calculateAiCoverage(aiRanges: number[][], nodeStart: number, nodeEnd: number): number {
    let aiCoverage = 0;
...
export function migrateAiRangesToMetadata(
    ...
    console.log(`Migration: Migrating ${aiRanges.length} aiRanges to OriginTextNode metadata`);
    ...
    editor.update(() => {
        ...
        const processNode = (node: LexicalNode) => {
            ...
            if (node instanceof TextNode) {
                ...
                const aiCoverage = calculateAiCoverage(aiRanges, nodeStart, nodeEnd);
                const textLength = text.length;

                // If more than 50% is AI, mark as AI; otherwise user
                const isAi = textLength > 0 && (aiCoverage / textLength) > 0.5;
                const newOrigin = isAi ? 'ai' : 'user';

                // If it's already an OriginTextNode, just update origin
                if ($isOriginTextNode(node)) {
                    const currentOrigin = (node as OriginTextNode).getOrigin();
                    ...
                    const originNode = $createOriginTextNode(text, newOrigin);
        ...
        console.log(`Migration: Updated ${migratedNodes} nodes, converted ${convertedNodes} TextNodes to OriginTextNodes`);
 ...
 */
export function needsMigration(aiRanges: number[][] | undefined | null): boolean {
