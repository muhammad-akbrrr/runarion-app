...
export default function AuditorToolsTab({ 
    ...
    const [selectedStoryIssues, setSelectedStoryIssues] = useState<Set<number>>(new Set());
    const [applyingStoryFix, setApplyingStoryFix] = useState<number | null>(null);
    const [applyingAllStory, setApplyingAllStory] = useState(false);
    const [expandedStoryIssues, setExpandedStoryIssues] = useState<Set<number>>(new Set());
    ...
    const [availableEntities, setAvailableEntities] = useState<Record<string, Array<{vertex_id: string; name: string}>>>({});
    const [selectedCategories, setSelectedCategories] = useState<Set<string>>(new Set());
    const [selectedEntities, setSelectedEntities] = useState<Set<string>>(new Set()); // vertex_ids
    const [selectedChapters, setSelectedChapters] = useState<Set<number>>(new Set());
    const [refreshMode, setRefreshMode] = useState<'all' | 'category' | 'entity'>('all');
    const [storyCheckMode, setStoryCheckMode] = useState<'all' | 'selected'>('all');
    const [recordCheckMode, setRecordCheckMode] = useState<'all' | 'category' | 'entity'>('all');
    const [duplicateCheckMode, setDuplicateCheckMode] = useState<'all' | 'category' | 'entity'>('all');
    ...
    const loadEntitiesForCategory = async (category: string) => {
        ...
        try {
            const url = `/${workspaceId}/projects/${projectId}/editor/records/entities?category=${encodeURIComponent(category)}`;
            ...
            if (response.ok) {
                const data = await response.json();
                console.log(`[Auditor] Got ${data.entities?.length || 0} entities for ${category}`);
                ...
                console.error(`[Auditor] Failed to load entities: ${response.status}`);
            }
        } catch (error) {
    ...
    const handleRecordConsistencyCheck = async () => {
        ...
        const targetCategories = recordCheckMode === 'category' ? Array.from(selectedCategories) : null;
        const targetEntities = recordCheckMode === 'entity' ? Array.from(selectedEntities) : null;
    ...
    const handleStoryConsistencyCheck = async () => {
        ...
        const targetChapters = storyCheckMode === 'selected' ? Array.from(selectedChapters) : null;
    ...
    const handleFindDuplicates = async () => {
        ...
        const targetCategories = duplicateCheckMode === 'category' ? Array.from(selectedCategories) : null;
        const targetEntities = duplicateCheckMode === 'entity' ? Array.from(selectedEntities) : null;
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (errors.length > 0 && (data.results?.potential_duplicates || []).length === 0) {
                    // Check if it's rate limit error
                    const hasRateLimit = errors.some((e: string) => 
                        e.includes('429') || e.includes('quota') || e.includes('RESOURCE_EXHAUSTED')
                    );
                    if (hasRateLimit) {
                        alert("API rate limit exceeded. Please wait and try again, or use a paid API key.");
                    }
                }
            } else {
                const errorData = await response.json();
                const errorMsg = errorData.details?.error || errorData.error || "Unknown error";
    ...
    const handleMergeEntities = async (group: DuplicateGroup) => {
        ...
        const source = group.entities.find(e => e.name !== group.suggested_canonical);
        const target = group.entities.find(e => e.name === group.suggested_canonical) || group.entities[0];
        ...
        try {
            ...
            if (response.ok) {
                ...
                setDuplicates(prev => prev.filter(g => 
    ...
    const handleApplyFix = async (issue: ConsistencyIssue, index: number, skipPreview = false) => {
                ...
                if (response.ok) {
                    ...
                    if (data.success && results) {
                        ...
                        setPreviewData({
                            ...
                            oldValue: typeof results.old_value === 'object' ? JSON.stringify(results.old_value, null, 2) : String(results.old_value ?? ''),
                            newValue: typeof results.new_value === 'object' ? JSON.stringify(results.new_value, null, 2) : String(results.new_value ?? ''),
                            explanation: results.explanation || results.message || ''
                        });
                        setEditedNewValue(typeof results.new_value === 'object' ? JSON.stringify(results.new_value, null, 2) : String(results.new_value ?? ''));
                        ...
                        setRecordIssues(prev => prev.filter((_, i) => i !== index));
                        ...
                        alert(`Failed to apply fix: ${data.error || results?.error || "Unknown error"}`);
                    }
                } else {
                    const error = await response.json();
    ...
    const handleApplyStoryTextFix = async (issue: ConsistencyIssue, index: number) => {
            ...
            if (response.ok) {
                ...
                if (data.success) {
                    ...
                    setStoryTextPreview({
                        ...
                        chapterName: results.chapter_name || issue.location || 'Unknown Chapter',
                ...
                const error = await response.json();
                alert(`Failed to generate fix: ${error.error || "Unknown error"}`);
            }
        } catch (error) {
    ...
    const confirmStoryTextFix = async () => {
        ...
        try {
            ...
            console.log('[AuditorFix] Old text length:', storyTextPreview.oldText?.length);
            console.log('[AuditorFix] New text length:', editedStoryText?.length);
            ...
            if (onApplyStoryFix) {
                ...
                const success = onApplyStoryFix(storyTextPreview.oldText, editedStoryText);
                
                if (success) {
                    console.log('[AuditorFix] Callback succeeded - editor updated!');
                    // Show success message
                    alert(`✅ Fix applied to ${storyTextPreview.chapterName}! The editor has been updated.`);
                } else {
                    console.warn('[AuditorFix] Callback failed - text not found in editor');
                    // Show warning but don't fail - might be a sync issue
                    alert(`⚠️ Could not find text in the current editor. The text may have changed. Please try refreshing and applying again.`);
                }
            } else {
                // No callback - fall back to direct API save
                console.log('[AuditorFix] No callback, using direct API save...');
                ...
                if (chaptersResponse.ok) {
                    ...
                    const chapter = chapters.find((c: any) => c.order === chapterOrder) || chapters[0];
                    ...
                    if (chapter && chapter.content) {
                        ...
                        if (chapter.content.includes(storyTextPreview.oldText)) {
                            newContent = chapter.content.replace(storyTextPreview.oldText, editedStoryText);
                        } else {
                            const normalizedContent = normalizeText(chapter.content);
                            const normalizedOldText = normalizeText(storyTextPreview.oldText);
                            
                            if (normalizedContent.includes(normalizedOldText)) {
                                // Use approximate replacement based on position
                                const idx = normalizedContent.indexOf(normalizedOldText);
                                const ratio = chapter.content.length / normalizedContent.length;
                                const start = Math.floor(idx * ratio);
                                const end = Math.ceil((idx + normalizedOldText.length) * ratio);
                                newContent = chapter.content.substring(0, start) + editedStoryText + chapter.content.substring(end);
                            }
                        }
                        ...
                        if (newContent && newContent !== chapter.content) {
                            ...
                            alert(`✅ Fix applied to ${storyTextPreview.chapterName}! Please refresh to see changes.`);
                        } else {
                            alert("⚠️ Text not found in chapter. The fix may have already been applied or the text has changed.");
                        }
                    }
                }
            }
            ...
            // Remove from list
            setStoryIssues(prev => prev.filter((_, i) => i !== storyTextPreview.index));
    ...
    const handleApplyAllStorySelected = async () => {
        ...
        if (!confirm(`Generate fixes for ${fixableCount} selected issues?\n\nNote: Each fix will be shown for review before applying.`)) {
            return;
        }
    ...
    const handleRefreshAllProperties = async () => {
        const targetCategories = refreshMode === 'category' ? Array.from(selectedCategories) : null;
        const targetEntities = refreshMode === 'entity' ? Array.from(selectedEntities) : null;
        
        let message = "This will update ALL entity properties to reflect their current state in the story.\n\nCharacters who have changed (e.g., brave → cowardly) will have their properties updated.\n\nPrevious values will be backed up.\n\nContinue?";
        ...
        if (refreshMode === 'category') {
            ...
            message = `This will update ${targetEntities?.length} selected entit${targetEntities?.length === 1 ? 'y' : 'ies'}.\n\nPrevious values will be backed up.\n\nContinue?`;
        }
            ...
            if (response.ok) {
                ...
                try {
                    ...
                    if (errorMsg.includes("429") || errorMsg.includes("quota") || errorMsg.includes("RESOURCE_EXHAUSTED")) {
                        alert("API rate limit exceeded. Please wait a minute and try again, or use a paid API key.");
                    } else {
    ...
    return (
        <div className="space-y-4">
            <Accordion type="multiple" defaultValue={["scan-status"]} className="w-full">
                {/* Scan Status Section */}
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {scanStatus && (
                                <Badge variant="outline" className="ml-2">
                                    {scanStatus.chapters_with_changes + scanStatus.chapters_not_scanned} pending
                                </Badge>
                            )}
                        </div>
                    </AccordionTrigger>
                        ...
                        <div className="space-y-3 pt-2">
                            <div className="flex items-center justify-between">
                                <p className="text-xs text-gray-500">
                                    Shows which chapters have been scanned and which have changes.
                                </p>
                                <Button 
                                    size="sm" 
                                    variant="outline" 
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-4 gap-2 text-center">
                                        <div className="p-2 bg-gray-50 rounded">
                                            <div className="text-lg font-bold">{scanStatus.total_chapters}</div>
                                            <div className="text-xs text-gray-500">Chapters</div>
                                        </div>
                                        <div className="p-2 bg-blue-50 rounded">
                                            <div className="text-lg font-bold text-blue-700">{scanStatus.extraction_pending || scanStatus.chapters_not_scanned || 0}</div>
                                            <div className="text-xs text-blue-600">Extract</div>
                                        </div>
                                        <div className="p-2 bg-orange-50 rounded">
                                            <div className="text-lg font-bold text-orange-700">{scanStatus.summarization_pending || 0}</div>
                                            <div className="text-xs text-orange-600">Summary</div>
                                        </div>
                                        {(scanStatus.data_warnings || 0) > 0 && (
                                            <div className="p-2 bg-red-50 rounded">
                                                <div className="text-lg font-bold text-red-700">{scanStatus.data_warnings}</div>
                                                <div className="text-xs text-red-600">⚠️ Warnings</div>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="max-h-64 overflow-y-auto space-y-2">
                                        {Object.values(scanStatus.chapters).sort((a, b) => a.chapter_order - b.chapter_order).map((ch) => (
                                            <div 
                                                key={ch.chapter_order}
                                                className={`p-2 rounded border text-xs ${
                                                    ch.has_changes ? 'border-yellow-300 bg-yellow-50' : 'border-green-300 bg-green-50'
                                                }`}
                                            >
                                                <div className="flex items-center justify-between mb-1">
                                                    <span className="font-medium">Ch. {ch.chapter_order + 1}: {ch.chapter_name}</span>
                                                    {ch.has_changes && (
                                                        <Badge variant="outline" className="text-[10px] bg-yellow-100 text-yellow-700">Changed</Badge>
                                                    )}
                                                </div>
                                                ...
                                                <div className="grid grid-cols-2 gap-1 mt-1">
                                                    ...
                                                    <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded ${
                                                        ...
                                                        <span>Extract</span>
                                                        {ch.extraction?.categories_extracted?.length > 0 && (
                                                            <span className="text-[10px]">({ch.extraction.categories_extracted.length})</span>
                                                        )}
                                                    </div>
                                                ...
                                                {/* Warning messages */}
                                                {(ch.extraction?.warning || ch.record_keeper?.warning) && (
                                                    <div className="mt-1 text-[10px] text-red-600 bg-red-50 p-1 rounded">
                                                        {ch.extraction?.warning && <div>⚠️ {ch.extraction.warning}</div>}
                                                        {ch.record_keeper?.warning && <div>⚠️ {ch.record_keeper.warning}</div>}
                                                    </div>
                                                )}
                                                ...
                                                {ch.extraction?.categories_extracted?.length > 0 && (
                                                    <div className="mt-1 text-[10px] text-gray-500">
                                                        Categories: {ch.extraction.categories_extracted.join(', ')}
                                                    </div>
                                                )}
                                            </div>
                                        ))}
                                    ...
                                    <p className="text-xs text-gray-500 italic">
                                        💡 Use Extractor for entities, Summarizer for Record Keeper entries.
                                    </p>
                                </>
                            )}
                        </div>
                ...
                <AccordionItem value="record-consistency">
                    ...
                    <AccordionContent>
                        <div className="space-y-3 pt-2">
                            <p className="text-xs text-gray-500">
                                Compares your database records against story content to find contradictions, 
                                outdated info, or missing updates.
                            </p>
                            ...
                            <div className="p-2 border rounded space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex gap-1">
                                        <Button
                                            variant={recordCheckMode === 'all' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setRecordCheckMode('all')}
                                        >
                                            All
                                        </Button>
                                        <Button
                                            variant={recordCheckMode === 'category' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setRecordCheckMode('category')}
                                        >
                                            Category
                                        </Button>
                                        <Button
                                            variant={recordCheckMode === 'entity' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setRecordCheckMode('entity')}
                                        >
                                            Entity
                                        </Button>
                                    </div>
                                                ...
                                                <div key={cat} className="border rounded">
                                                    ...
                                                    {expandedCategory === cat && (
                                                        <div className="border-t p-1.5 bg-gray-50 space-y-1">
                                                            ...
                                                            ) : (availableEntities[cat] || []).length === 0 ? (
                                                                ...
                                                                <>
                                                                    <div className="flex gap-2 mb-1">
                                                                        <Button
                                                                            ...
                                                                            onClick={() => selectAllEntitiesInCategory(cat)}
                                                                        >
                                                                            Select All
                                                                        </Button>
                                                                        <Button
                                                                            ...
                                                                            size="sm"
                                                                            className="h-5 text-xs px-1"
                                                                            onClick={() => deselectAllEntitiesInCategory(cat)}
                                                                        >
                                                                            Deselect All
                                                                        </Button>
                                                                    </div>
                                                                    {(availableEntities[cat] || []).map(entity => (
                                                                        <label 
                                                                            key={entity.vertex_id}
                                                                            className={`flex items-center gap-1.5 text-xs cursor-pointer p-1 rounded ${
                                                                                selectedEntities.has(entity.vertex_id) 
                                                                                    ? 'bg-orange-100 text-orange-800' 
                                                                                    : 'hover:bg-gray-100'
                                                                            }`}
                                                                        >
                                                                            <Checkbox
                                                                                checked={selectedEntities.has(entity.vertex_id)}
                                                                                onCheckedChange={(checked) => {
                                                                                    const next = new Set(selectedEntities);
                                                                                    if (checked) next.add(entity.vertex_id);
                                                                                    else next.delete(entity.vertex_id);
                                ...
                                {recordCheckMode === 'entity' && selectedEntities.size > 0 && (
                                    <div className="text-xs text-orange-600">
                                        {selectedEntities.size} entit{selectedEntities.size === 1 ? 'y' : 'ies'} selected
                                    </div>
                                )}
                            </div>
                            ...
                            >
                                {loadingRecordCheck ? (
                                    ...
                                    <>
                                        ...
                                        {recordCheckMode === 'category' && `Check ${selectedCategories.size} Categor${selectedCategories.size === 1 ? 'y' : 'ies'}`}
                                        {recordCheckMode === 'entity' && `Check ${selectedEntities.size} Entit${selectedEntities.size === 1 ? 'y' : 'ies'}`}
                                    </>
                                )}
                            </Button>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-medium text-orange-700">
                                            {recordIssues.length} issue{recordIssues.length !== 1 ? 's' : ''} found
                                        </span>
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                    ...
                                    <div className="flex items-center justify-between gap-2 p-2 bg-gray-50 rounded">
                                        <div className="flex items-center gap-2">
                                            <Checkbox 
                                                checked={selectedIssues.size === recordIssues.length && recordIssues.length > 0}
                                            ...
                                            <div key={idx} className={`p-2 rounded border ${getSeverityColor(issue.severity)}`}>
                                                <div className="flex items-start gap-2">
                                                    ...
                                                    <Checkbox 
                                                        ...
                                                        onCheckedChange={() => toggleIssueSelection(idx)}
                                                        className="mt-0.5"
                                                    />
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <Badge className={`${getIssueTypeColor(issue.issue_type)} text-white text-xs`}>
                                                                {issue.issue_type?.replace('_', ' ')}
                                                            </Badge>
                                                            <span className="font-medium text-sm truncate">{issue.entity_name}</span>
                                                            <span className="text-[10px] text-gray-400 ml-auto">
                                                                {expandedIssues.has(idx) ? '▼' : '▶'}
                                                            </span>
                                                        </div>
                                                        {issue.field && (
                                                            <p className="text-xs"><strong>Field:</strong> {issue.field}</p>
                                                        )}
                                                        {issue.story_evidence && (
                                                            <p className={`text-xs mt-1 italic text-orange-700 ${expandedIssues.has(idx) ? '' : 'line-clamp-2'}`}>
                                                                "{issue.story_evidence}"
                                                            </p>
                                                        )}
                                                        {issue.suggestion && (
                                                            <p className={`text-xs mt-1 text-gray-600 ${expandedIssues.has(idx) ? '' : 'line-clamp-2'}`}>
                                                                💡 {issue.suggestion}
                                                            </p>
                                                        )}
                                                    </div>
                                                    ...
                                                    <Button
                                                        ...
                                                        disabled={applyingFix === idx || applyingAll}
                ...
                <AccordionItem value="story-consistency">
                    ...
                    <AccordionContent>
                        <div className="space-y-3 pt-2">
                            <p className="text-xs text-gray-500">
                                Analyzes your story for plot holes, timeline issues, character inconsistencies, 
                            ...
                            <div className="p-2 border rounded space-y-2">
                                <div className="flex items-center justify-between">
                                    <Label className="text-xs font-medium">Select Chapters:</Label>
                                    <div className="flex gap-1">
                                        <Button
                                            variant={storyCheckMode === 'all' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                        ...
                                        <Button
                                            variant={storyCheckMode === 'selected' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setStoryCheckMode('selected')}
                                        >
                                            Select
                                        </Button>
                                    </div>
                                ...
                                {storyCheckMode === 'selected' && scanStatus && (
                                    <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto">
                                        {Object.values(scanStatus.chapters).map(ch => (
                                            <label 
                                                key={ch.chapter_order} 
                                                className={`flex items-center gap-1 px-2 py-1 rounded text-xs cursor-pointer transition-colors ${
                                                    selectedChapters.has(ch.chapter_order) 
                                                        ? 'bg-purple-100 text-purple-800 border border-purple-300' 
                                                        : 'bg-gray-100 text-gray-600 border border-gray-200 hover:bg-gray-200'
                                                }`}
                                            >
                                                <Checkbox
                                                    checked={selectedChapters.has(ch.chapter_order)}
                                                    onCheckedChange={(checked) => {
                                                        const next = new Set(selectedChapters);
                                                        if (checked) next.add(ch.chapter_order);
                                                        else next.delete(ch.chapter_order);
                                                        setSelectedChapters(next);
                                                    }}
                                                    className="h-3 w-3"
                                                />
                                                {ch.chapter_name || `Ch ${ch.chapter_order + 1}`}
                                            </label>
                                        ))}
                                    </div>
                                )}
                            ...
                            <Button 
                                ...
                                disabled={loadingStoryCheck || (storyCheckMode === 'selected' && selectedChapters.size === 0)}
                                className="w-full"
                            >
                                {loadingStoryCheck ? (
                                    <>
                                    ...
                                    <>
                                        ...
                                        {storyCheckMode === 'all' 
                                            ...
                                            : `Check ${selectedChapters.size} Chapter${selectedChapters.size === 1 ? '' : 's'}`
                                        }
                                    </>
                                )}
                            </Button>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-medium text-purple-700">
                                            {storyIssues.length} issue{storyIssues.length !== 1 ? 's' : ''} found
                                        </span>
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                    ...
                                    <div className="flex items-center justify-between gap-2 p-2 bg-gray-50 rounded">
                                        <div className="flex items-center gap-2">
                                            <Checkbox 
                                                checked={selectedStoryIssues.size === storyIssues.length && storyIssues.length > 0}
                                                onCheckedChange={() => {
                                            ...
                                            />
                                            <span className="text-xs text-gray-600">
                                                {selectedStoryIssues.size > 0 ? `${selectedStoryIssues.size} selected` : 'Select all'}
                                            </span>
                                        </div>
                                        {selectedStoryIssues.size > 0 && (
                                            <Button
                                                size="sm"
                                                variant="default"
                                                onClick={handleApplyAllStorySelected}
                                            ...
                                            >
                                                {applyingAllStory ? (
                                                    ...
                                                    <>
                                                        <CheckSquare className="h-3 w-3 mr-1" />
                                                        Fix Selected ({selectedStoryIssues.size})
                                                    </>
                                                )}
                                            </Button>
                                        )}
                                        ...
                                        {storyIssues.map((issue, idx) => (
                                            <div key={idx} className={`p-2 rounded border ${getSeverityColor(issue.severity)}`}>
                                                <div className="flex items-start gap-2">
                                                    {/* Checkbox */}
                                                    <Checkbox 
                                                        checked={selectedStoryIssues.has(idx)}
                                                        onCheckedChange={() => {
                                                            setSelectedStoryIssues(prev => {
                                                                const next = new Set(prev);
                                                                if (next.has(idx)) {
                                                    ...
                                                    <div 
                                                        className="flex-1 min-w-0 cursor-pointer"
                                                        onClick={() => setExpandedStoryIssues(prev => {
                                                            const next = new Set(prev);
                                                            if (next.has(idx)) {
                                                                next.delete(idx);
                                                            } else {
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-2 mb-1">
                                                            ...
                                                            <span className="text-[10px] text-gray-400 ml-auto">
                                                                {expandedStoryIssues.has(idx) ? '▼' : '▶'}
                                                            </span>
                                                        </div>
                                                        <p className="font-medium text-sm">{issue.title || issue.entity_name}</p>
                                                        {issue.description && (
                                                            <p className={`text-xs mt-1 ${expandedStoryIssues.has(idx) ? '' : 'line-clamp-2'}`}>
                                                                {issue.description}
                                                            </p>
                                                        )}
                                                        {issue.evidence && (
                                                            <p className={`text-xs mt-1 italic text-gray-600 ${expandedStoryIssues.has(idx) ? '' : 'line-clamp-2'}`}>
                                                                "{issue.evidence}"
                                                            </p>
                                                        )}
                                                        {issue.suggestion && (
                                                            <p className={`text-xs mt-1 text-gray-600 ${expandedStoryIssues.has(idx) ? '' : 'line-clamp-2'}`}>
                                                                💡 {issue.suggestion}
                                                            </p>
                                                        )}
                                                    </div>
                                                    ...
                                                    <Button
                                                        size="sm"
                                                        variant="outline"
                                                        onClick={() => handleApplyStoryTextFix(issue, idx)}
                                                        disabled={applyingStoryFix === idx || applyingAllStory}
                                                        className="h-7 text-xs shrink-0"
                                                    >
                                                        {applyingStoryFix === idx ? (
                                                            <Loader2 className="h-3 w-3 animate-spin" />
                                                        ) : (
                                                            <>
                                                                <Wand2 className="h-3 w-3 mr-1" />
                ...
                <AccordionItem value="property-refresh">
                    ...
                    <AccordionContent>
                        <div className="space-y-3 pt-2">
                            <p className="text-xs text-gray-500">
                                <strong>Arc-Aware Updates:</strong> Updates entity properties to reflect their 
                                <em> current state</em> in the story. Characters who have changed (brave → cowardly, 
                                determined → broken) will be updated. Previous values are backed up.
                            </p>
                            
                            <div className="p-2 bg-indigo-50 rounded text-xs space-y-1">
                                <div className="flex items-center gap-1">
                                    <TrendingUp className="h-3 w-3 text-indigo-600" />
                                    <span className="font-medium text-indigo-800">How it works:</span>
                                </div>
                                <ul className="list-disc list-inside text-indigo-700 space-y-0.5 ml-1">
                                    ...
                                    <li>Determines each entity's <em>current</em> state</li>
                            ...
                            <div className="p-2 border rounded space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex gap-1">
                                        <Button
                                            variant={refreshMode === 'all' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setRefreshMode('all')}
                                        >
                                            All
                                        </Button>
                                        <Button
                                            variant={refreshMode === 'category' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setRefreshMode('category')}
                                        >
                                            Category
                                        </Button>
                                        <Button
                                            variant={refreshMode === 'entity' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setRefreshMode('entity')}
                                        >
                                            Entity
                                        </Button>
                                    </div>
                                ...
                                {(refreshMode === 'category' || refreshMode === 'entity') && (
                                    <div className="space-y-1 max-h-40 overflow-y-auto">
                                        {loadingCategories ? (
                                            <span className="text-xs text-gray-400">Loading categories...</span>
                                        ) : availableCategories.length === 0 ? (
                                            ...
                                            availableCategories.map(cat => (
                                                <div key={cat} className="border rounded">
                                                    <div 
                                                        className={`flex items-center justify-between p-1.5 cursor-pointer hover:bg-gray-50 ${
                                                            selectedCategories.has(cat) ? 'bg-indigo-50' : ''
                                                        }`}
                                                        onClick={() => {
                                                            if (refreshMode === 'entity') {
                                                    ...
                                                    >
                                                        <label className="flex items-center gap-1.5 text-xs cursor-pointer">
                                                            ...
                                                            />
                                                            <span className={selectedCategories.has(cat) ? 'text-indigo-700 font-medium' : ''}>
                                                                {cat}
                                                            </span>
                                                        </label>
                                                        {refreshMode === 'entity' && (
                                                    ...
                                                    {refreshMode === 'entity' && expandedCategory === cat && (
                                                        <div className="border-t p-1.5 bg-gray-50 space-y-1">
                                                            {loadingEntities ? (
                                                                <span className="text-xs text-gray-400">Loading...</span>
                                                            ) : (availableEntities[cat] || []).length === 0 ? (
                            ...
                            >
                                {loadingRefresh ? (
                                    ...
                                    <>
                                        ...
                                        {refreshMode === 'category' && `Refresh ${selectedCategories.size} Categor${selectedCategories.size === 1 ? 'y' : 'ies'}`}
                                        {refreshMode === 'entity' && `Refresh ${selectedEntities.size} Entit${selectedEntities.size === 1 ? 'y' : 'ies'}`}
                                    </>
                                )}
                            </Button>
                            ...
                            {loadingRefresh && (
                                <div className="p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-700">
                                    <strong>Processing in batches...</strong> Each entity is analyzed against your story. 
                                    For large databases, this can take 2-5 minutes. Please don't close this tab.
                                </div>
                            )}
                            ...
                            {refreshResults && (
                                <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-3 gap-2 text-center">
                                        <div className="p-2 bg-gray-50 rounded">
                                            <div className="text-lg font-bold">{refreshResults.entities_processed}</div>
                                            <div className="text-xs text-gray-500">Processed</div>
                                        </div>
                                        <div className="p-2 bg-indigo-50 rounded">
                                            <div className="text-lg font-bold text-indigo-700">{refreshResults.entities_updated}</div>
                                            <div className="text-xs text-indigo-600">Updated</div>
                                        </div>
                                        <div className="p-2 bg-green-50 rounded">
                                            <div className="text-lg font-bold text-green-700">{refreshResults.entities_unchanged}</div>
                                            <div className="text-xs text-green-600">Current</div>
                                        </div>
                                    </div>
                                    
                                    {Object.keys(refreshResults.changes_by_entity).length > 0 && (
                                        <div className="max-h-48 overflow-y-auto space-y-2">
                                            {Object.entries(refreshResults.changes_by_entity).map(([entityName, changes]) => (
                                                <div key={entityName} className="p-2 rounded border bg-white">
                                                    <div className="font-medium text-sm mb-1">{entityName}</div>
                                                    {changes.map((change, idx) => (
                                                        <div key={idx} className="text-xs bg-gray-50 p-1.5 rounded mt-1">
                                                            <div className="flex items-center gap-1">
                                                                <span className="font-medium">{change.field}:</span>
                                                                <span className="text-red-600 line-through">{String(change.old_value).slice(0, 30)}</span>
                                                                <span className="text-gray-400">→</span>
                                                                <span className="text-green-600">{String(change.new_value).slice(0, 30)}</span>
                                                            </div>
                                                            {change.reason && (
                                                                <div className="text-gray-500 mt-0.5 italic">
                                                                    {change.chapter_reference && `Ch. ${change.chapter_reference}: `}
                                                                    {change.reason}
                                                                </div>
                                                            )}
                                                        </div>
                            ...
                            {!refreshResults && !loadingRefresh && (
                                <p className="text-xs text-gray-500 italic text-center">
                                    Run refresh to update entity properties to current story state
                                </p>
                            )}
                        </div>
                    </AccordionContent>
                ...
                <AccordionItem value="duplicates">
                    ...
                    <AccordionContent>
                        <div className="space-y-3 pt-2">
                            <p className="text-xs text-gray-500">
                                Find potential duplicate entities (same entity with different names) 
                                and merge them to clean up your database.
                            </p>
                            ...
                            <div className="p-2 border rounded space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex gap-1">
                                        <Button
                                            variant={duplicateCheckMode === 'all' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setDuplicateCheckMode('all')}
                                        >
                                            All
                                        </Button>
                                        <Button
                                            variant={duplicateCheckMode === 'category' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setDuplicateCheckMode('category')}
                                        >
                                            Category
                                        </Button>
                                        <Button
                                            variant={duplicateCheckMode === 'entity' ? 'default' : 'outline'}
                                            size="sm"
                                            className="h-6 text-xs px-2"
                                            onClick={() => setDuplicateCheckMode('entity')}
                                        >
                                            Entity
                                        </Button>
                                    </div>
                            ...
                            >
                                {loadingDuplicates ? (
                                    ...
                                    <>
                                        ...
                                        {duplicateCheckMode === 'category' && `Find in ${selectedCategories.size} Categor${selectedCategories.size === 1 ? 'y' : 'ies'}`}
                                        {duplicateCheckMode === 'entity' && `Check ${selectedEntities.size} Entit${selectedEntities.size === 1 ? 'y' : 'ies'}`}
                                    </>
                                )}
                            </Button>
                                ...
                                <div className="space-y-2 max-h-64 overflow-y-auto">
                                    {duplicates.map((group, idx) => (
                                        <div key={idx} className="p-2 rounded border bg-gray-50">
                                            ...
                                            <div className="flex items-center gap-1 mb-1">
                                                {group.entities.map((e, i) => (
                                                    <span key={e.vertex_id}>
                                                        <span className={`text-sm ${e.name === group.suggested_canonical ? 'font-bold' : ''}`}>
                                                            {e.name}
                                                        </span>
                                            ...
                                            <p className="text-xs text-gray-600 mb-2">{group.reason}</p>
                                            <Button 
                                                size="sm" 
                                                variant="outline"
                            ...
                            {duplicateErrors.length > 0 && (
                                <div className="p-2 rounded border border-red-200 bg-red-50 text-xs">
                                    <p className="font-medium text-red-700 mb-1">Errors during scan:</p>
                                    <ul className="list-disc list-inside text-red-600 space-y-0.5">
                                        {duplicateErrors.map((err, i) => (
                                            <li key={i} className="truncate" title={err}>
                                                {err.includes('429') || err.includes('RESOURCE_EXHAUSTED') 
                            ...
                            {duplicates.length === 0 && duplicateErrors.length === 0 && !loadingDuplicates && (
                                <p className="text-xs text-gray-500 italic text-center">
            ...
            <Dialog open={previewData !== null} onOpenChange={(open) => !open && setPreviewData(null)}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            The following change was made to <strong>{previewData?.issue.entity_name}</strong>
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    {previewData && (
                        <div className="space-y-4">
                            <div>
                                <Label className="text-xs text-gray-500">Field Updated</Label>
                                <p className="font-medium text-blue-700">{previewData.field}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="p-2 bg-red-50 rounded text-sm font-mono border border-red-200 max-h-24 overflow-y-auto">
                                    {previewData.oldValue || <span className="text-gray-400 italic">(no value)</span>}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="p-2 bg-green-50 rounded text-sm font-mono border border-green-200 max-h-24 overflow-y-auto">
                                    {previewData.newValue || <span className="text-gray-400 italic">(no value)</span>}
                                </div>
                            </div>
                            ...
                            {previewData.explanation && (
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600 italic bg-blue-50 p-2 rounded">{previewData.explanation}</p>
                                </div>
                            )}
                            ...
                            {previewData.oldValue === previewData.newValue && (
                                <div className="p-2 bg-yellow-50 rounded border border-yellow-200 text-sm text-yellow-700">
                                    ⚠️ No change was needed - the value was already correct.
                                </div>
                            )}
                        </div>
                    )}
            ...
            <Dialog open={storyTextPreview !== null} onOpenChange={(open) => !open && setStoryTextPreview(null)}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Review the suggested text change for <strong>{storyTextPreview?.chapterName}</strong>
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    {storyTextPreview && (
                        <div className="space-y-4 flex-1 overflow-y-auto">
                            <div className="p-2 bg-purple-50 rounded border border-purple-200">
                                <Label className="text-xs text-purple-700 font-medium">Issue: {storyTextPreview.issue.title}</Label>
                                {storyTextPreview.issue.description && (
                                    <p className="text-xs text-purple-600 mt-1">{storyTextPreview.issue.description}</p>
                                )}
                            </div>
                            
                            <div>
                                <Label className="text-xs text-gray-500 flex items-center gap-1">
                                    <span className="text-red-500">●</span> Original Text (will be replaced)
                                </Label>
                                <div className="p-3 bg-red-50 rounded text-sm font-mono border border-red-200 max-h-32 overflow-y-auto whitespace-pre-wrap">
                                    {storyTextPreview.oldText || <span className="text-gray-400 italic">(text not found)</span>}
                                </div>
                            </div>
                            
                            <div>
                                <Label className="text-xs text-gray-500 flex items-center gap-1">
                                    <span className="text-green-500">●</span> Revised Text (editable)
                                </Label>
                                <Textarea
                            ...
                            {storyTextPreview.explanation && (
                                <div>
                                    <Label className="text-xs text-gray-500">AI Explanation</Label>
                                    <p className="text-sm text-gray-600 italic bg-blue-50 p-2 rounded">{storyTextPreview.explanation}</p>
                                </div>
                            )}
                            
                            <div className="p-2 bg-yellow-50 rounded border border-yellow-200 text-xs text-yellow-700">
                                <strong>💡 Tip:</strong> The revised text will replace the original text in your chapter. 
                                You can edit it before applying.
                            </div>
                        </div>
                    )}
                    
                    <DialogFooter className="gap-2">
                        <Button variant="outline" onClick={() => setStoryTextPreview(null)}>
                            Cancel
                        </Button>
                        <Button 
