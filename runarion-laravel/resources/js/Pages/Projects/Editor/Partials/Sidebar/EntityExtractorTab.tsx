...
export default function EntityExtractorTab({ 
    ...
    const [scanMode, setScanMode] = useState<"incremental" | "full" | "new_only">("incremental");
    ...
    }, [selectedCategories, useAllCategories, selectedChapters, useAllChapters, chapters.length, collectionTypes.length]);
    ...
    const calculateEstimatedTime = () => {
        ...
        const categoriesToProcess = useAllCategories ? collectionTypes.length : selectedCategories.length;
    ...
    const handleStart = () => {
        if (useAllCategories && collectionTypes.length === 0) {
            alert("No categories available. Please create some collection types first.");
            return;
        }
    ...
    const handleConfirm = async () => {
        ...
        try {
            const categories = useAllCategories ? ["all_categories"] : selectedCategories;
            
            const response = await fetch(
                `/${workspaceId}/projects/${projectId}/editor/auditor/extract`,
                {
                    method: "POST",
                    headers: {
            ...
            if (response.ok) {
                ...
                const categoriesProcessed = data.results?.categories_processed || {};
                ...
                if (created > 0 || updated > 0) {
                    ...
                    window.dispatchEvent(new CustomEvent('entities-extracted', { 
                        detail: { created, updated } 
                    }));
                }
            } else {
                const error = await response.json();
                let errorMessage = `Failed to extract entities: ${error.error || "Unknown error"}`;
                if (error.details) {
            ...
            alert(`Failed to extract entities: ${error?.message || String(error)}`);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-sm font-semibold text-blue-900">Entity Extractor</h3>
                    <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                            <p>
                                Automatically extracts entities from your manuscript content. 
                                Works with both system categories (Character, Location, etc.) 
                                and custom categories you create. Entities are created/updated 
                                in the Records System with properties extracted from the text.
                            </p>
                        </TooltipContent>
                    </Tooltip>
                </div>
                <p className="text-xs text-blue-700">
                    Scans your entire manuscript and extracts entities based on selected categories. 
                    Custom categories use their field schemas to extract relevant information dynamically.
                </p>
            </div>
            ...
            <div>
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                            <p>
                                Select which entity categories to extract. "All Categories" processes 
                                all system and custom categories sequentially. Custom categories will 
                                ...
                                collectionTypes.map((type) => (
                                    <div key={type.id} className="flex items-center space-x-2">
                                        <Checkbox
                                            ...
                                            onCheckedChange={() => toggleCategory(type.value)}
                                        />
                                        <Label 
            ...
            <div>
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                            <p>
                                Select which chapters to extract entities from. Usually you want 
                                all chapters to get a complete picture of all entities in your story.
                            </p>
                        </TooltipContent>
                    </Tooltip>
                </div>
                <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                        ...
                        />
                        <Label htmlFor="all-chapters-extract" className="text-sm cursor-pointer">
                ...
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <Tooltip>
                            ...
                            <TooltipContent className="max-w-xs">
                                <p>
                                    <strong>Incremental:</strong> Only process chapters that have changed or never been scanned. Saves time and API costs.
                                    <br /><br />
                                    <strong>Full Rescan:</strong> Process all chapters regardless of previous scans. Use when you want fresh extraction.
                                    <br /><br />
                                    <strong>New Only:</strong> Only process chapters that have never been scanned before.
                                </p>
                            </TooltipContent>
                        </Tooltip>
                    </div>
                    <Select value={scanMode} onValueChange={(value: "incremental" | "full" | "new_only") => setScanMode(value)}>
                        <SelectTrigger>
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                        {scanMode === "incremental" && "Processes only chapters with changes since last scan."}
                        {scanMode === "full" && "Rescans all chapters - useful if you want fresh extraction."}
                        {scanMode === "new_only" && "Only processes chapters that have never been scanned."}
                    </p>
                </div>
            )}
            ...
            <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <p className="text-xs text-blue-600 mt-1">
                    ...
                    } Entities are created or updated in the Records System. You can navigate 
                    away and will be notified when complete.
                </p>
            </div>
            ...
            <Button 
                onClick={handleStart} 
                disabled={loading || (!useAllCategories && selectedCategories.length === 0)}
                className="w-full"
            >
            ...
            <p className="text-xs text-gray-500 text-center">
                This will extract entities from your manuscript and create/update them 
                in the Records System based on selected categories.
            </p>
            ...
            <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            This will extract entities from your manuscript and create/update 
                            them in the Records System. You can navigate away and will be 
                            notified when complete.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-2 py-4">
                        <div className="text-sm">
                            <strong>Categories:</strong> {useAllCategories ? `All (${collectionTypes.length})` : `${selectedCategories.length} selected`}
                        </div>
                        <div className="text-sm">
