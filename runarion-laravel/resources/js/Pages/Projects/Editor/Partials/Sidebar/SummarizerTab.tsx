...
export default function SummarizerTab({
    ...
    const calculateEstimatedTime = () => {
        ...
        if (
            ...
            const categories = collectionTypes.filter(
                (ct) => ct.value !== "record_keeper"
            );
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const rkUpdated =
                    data.results?.record_keeper_entries_updated || 0;
    ...
    const getCategoryLabel = () => {
        if (!selectedCategory || selectedCategory === "none") return "";
        const category = collectionTypes.find(
            (ct) => ct.value === selectedCategory
        );
        return category
            ? category.name
            : selectedCategory
        ...
        <div className="space-y-6">
            ...
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Tooltip>
                            ...
                            <TooltipContent className="max-w-xs">
                                <p>
                                    <strong>Record Keeper:</strong> Analyzes
                                    your manuscript and creates Record Keeper
                                    entries for each chapter. Each entry
                                    includes a comprehensive summary, character
                                    activity, key events, themes, and locations
                                    mentioned.
                                </p>
                            </TooltipContent>
                        </Tooltip>
                    ...
                    <div className="flex items-center space-x-2">
                        <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                                setEnableRecordKeeper(checked as boolean)
                            }
                        />
                        <Label
                            htmlFor="enable-record-keeper"
                ...
                <p className="text-xs text-blue-700">
                    Creates chapter-by-chapter summaries stored as Record Keeper
                    entities. You can enable/disable this independently of
                    category summaries.
                </p>
            </div>
            ...
            <div>
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                            <p>
                                Generate chapter-by-chapter activity summaries
                                for entities. This creates/updates entity
                                properties with <code>_summaries</code> array
                                that appears in the Summary tab when you open an
                                entity. Works with both system categories
                                (Character, Location, etc.) and custom
                                categories (like Faction) you create.
                            </p>
                        </TooltipContent>
                    </Tooltip>
                </div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                    Select a category to generate entity summaries, or "All
                    Categories" to process all categories sequentially. Leave as
                    "None" to only run Record Keeper (if enabled).
                </p>
            </div>
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <Tooltip>
                                ...
                                <TooltipContent className="max-w-xs">
                                    <p>
                                        <strong>All:</strong> Process all{" "}
                                        {getCategoryLabel().toLowerCase()}{" "}
                                        entities across selected chapters.
                                        <br />
                                        <br />
                                        <strong>Focused:</strong> Select
                                        specific{" "}
                                        {getCategoryLabel().toLowerCase()}{" "}
                                        entities to analyze in detail.
                                    </p>
                                </TooltipContent>
                            </Tooltip>
                        </div>
                        <RadioGroup
                            value={analysisMode}
                            onValueChange={(value) =>
                                setAnalysisMode(value as "all" | "focused")
                            }
                        >
                            <div className="flex items-center space-x-2">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <Tooltip>
                            ...
                            <TooltipContent className="max-w-xs">
                                <p>
                                    Choose which{" "}
                                    {getCategoryLabel().toLowerCase()} entities
                                    to analyze. Only entities of the selected
                                    category are shown.
                                </p>
                            </TooltipContent>
                        </Tooltip>
                    ...
                    <div className="max-h-40 overflow-y-auto border rounded p-2 space-y-2">
                        {getFilteredEntities().length === 0 ? (
                            <p className="text-sm text-gray-500 italic">
                                No {getCategoryLabel().toLowerCase()} entities
                                found.
                            </p>
                        ) : (
            ...
            <div>
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                            <p>
                                Select which chapters to analyze. Processing
                                happens sequentially, one chapter at a time, to
                                handle large manuscripts efficiently.
                            </p>
                        </TooltipContent>
                    </Tooltip>
                </div>
            ...
            <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <p className="text-xs text-blue-600 mt-1">
                    ...
                    {enableRecordKeeper &&
                        " Record Keeper entries will be created for all selected chapters. "}
                    {selectedCategory &&
                        selectedCategory !== "none" &&
                        selectedCategory !== "all_categories" &&
                        `Entity summaries will be generated for ${getCategoryLabel()}. `}
                    {selectedCategory === "all_categories" &&
                        "All categories will be processed sequentially. "}
            ...
            <p className="text-xs text-gray-500 text-center">
                {enableRecordKeeper &&
                    "Will create Record Keeper entries for all selected chapters. "}
                {selectedCategory &&
                    selectedCategory !== "none" &&
                    selectedCategory !== "all_categories" &&
                    `Will generate summaries for ${getCategoryLabel()}. `}
                {selectedCategory === "all_categories" &&
                    "Will process all categories sequentially. "}
                {!enableRecordKeeper &&
                    !selectedCategory &&
            ...
            >
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            A terminal window will appear showing progress. You
                            can navigate away and will be notified when the
                    ...
                    <div className="space-y-2 py-4">
                        {enableRecordKeeper && (
                            <div className="text-sm">
                                <strong>Record Keeper:</strong> Will create
                                entries for all selected chapters
                            </div>
                        )}
                        {selectedCategory && selectedCategory !== "none" && (
                            <>
                                ...
                                {selectedCategory !== "all_categories" && (
                                    <div className="text-sm">
                                        ...
                                        {analysisMode === "all"
                                            ? `All ${getCategoryLabel()}s`
                                            : `Focused (${selectedEntities.length} selected)`}
                                    </div>
                                )}
                            </>
                        )}
