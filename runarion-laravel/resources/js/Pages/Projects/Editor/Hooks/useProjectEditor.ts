...
export function useProjectEditor({
    ...
    const [localChapters, setLocalChapters] =
        useState<ProjectChapter[]>(initialChapters);
    const [selectedChapter, setSelectedChapter] =
        useState<ProjectChapter | null>(null);
    const [preservedChapterOrder, setPreservedChapterOrder] = useState<
        number | null
    >(null);
    ...
    const {
        ...
        onContentSaved: (updatedChapters) => {
            ...
            if (selectedChapter) {
                const updatedSelectedChapter = updatedChapters.find(
                    (ch) => ch.order === selectedChapter.order,
                );
                if (updatedSelectedChapter) {
                    setSelectedChapter(updatedSelectedChapter);
                    originalContent.current =
                        updatedSelectedChapter.content || "";
                }
            }
        },
        onSettingsSaved: () => {
    ...
    const {
        ...
        onStreamComplete: (generatedText) => {
            console.log("Stream completed", {
                ...
                baseContentPreview:
                    baseContentRef.current?.substring(0, 50) || "(empty)",
            });
            ...
            setTimeout(() => {
                ...
                if (editorRef?.current) {
                    try {
                        ...
                        console.log(
                            "Stream complete - saving Lexical JSON with origin metadata:",
                            {
                        ...
                        if (currentChapter?.navigation_info) {
                            const newVersionIndex =
                                currentChapter.navigation_info
                        ...
                        console.error(
                            "Failed to get Lexical JSON from editor, falling back to plain text:",
                            error,
                        );
                    }
                }
                ...
                if (!savedSuccessfully) {
                    ...
                    const finalContent =
                        userPlainText + separator + generatedText;

                    console.log(
                        "FALLBACK: Final content composed as plain text (origin metadata lost):",
                        {
                    ...
                    if (currentChapter?.navigation_info) {
                        const newVersionIndex =
                            currentChapter.navigation_info.currentVersionIndex +
                            1;
                        versionControl.cacheVersion(
                            newVersionIndex,
                            finalContent,
                ...
                console.log(
                    "Stream completion save done, refreshing navigation info",
                );
    ...
    useEffect(() => {
        ...
        if (preservedChapterOrder !== null) {
            const chapterToRestore = initialChapters.find(
                (ch) => ch.order === preservedChapterOrder,
            );
            if (chapterToRestore) {
                console.log(
                    "Restoring chapter after generation:",
                    chapterToRestore.chapter_name,
                );
        ...
        if (selectedChapter) {
            const updatedChapter = initialChapters.find(
                (ch) => ch.order === selectedChapter.order,
            );
            if (updatedChapter) {
                // Check if navigation_info changed (compare JSON to handle object equality)
                const navigationChanged =
                    JSON.stringify(updatedChapter.navigation_info) !==
                ...
                console.log(
                    "Selected chapter was deleted, selecting first available chapter",
                );
    ...
    useEffect(() => {
        if (selectedChapter) {
            ...
            if (isChapterSwitch) {
                console.log(
                    "Initializing content for chapter:",
                    selectedChapter.chapter_name,
                );
                ...
                if (
                    needsMigration(selectedChapter.ai_ranges) &&
                    editorRef?.current
                ) {
                    console.log(
                        "Migrating legacy aiRanges to OriginTextNode metadata",
                    );
                ...
                localStorage.setItem(
                    storageKey,
                    selectedChapter.order.toString(),
                );

                // Initialize version control if needed
                if (!selectedChapter.navigation_info) {
                    initializeChapterHistory(
    ...
    useEffect(() => {
        ...
        const settingsChanged =
            ...
            JSON.stringify(originalSettings.current);

        if (settingsChanged || !isInitialized.current) {
            console.log("Initializing settings from project:", {
                settingsChanged,
        ...
        if (settingsChanged) {
            const changedKeys = Object.keys(settings).filter(
                (key) =>
                    ...
                    JSON.stringify(originalSettings.current[key]),
            );
            ...
            if (changedKeys.includes("aiModel")) {
                console.log(
                    "AI Model changed, saving immediately:",
                    settings.aiModel,
                );
    ...
    useEffect(() => {
        return () => {
            ...
            if (isInitialized.current) {
                ...
                if (settingsChanged) {
                    console.log(
                        "Component unmounting, force-saving settings immediately",
                    );
    ...
    useEffect(() => {
        if (
            ...
            if (lastChapterOrder !== null) {
                const lastChapter = localChapters.find(
                    (ch) => ch.order === parseInt(lastChapterOrder),
                );
                if (lastChapter) {
                    chapterToSelect = lastChapter;
                    console.log(
                        "Restored last viewed chapter:",
                        lastChapter.chapter_name,
                    );
                } else {
                    console.log(
                        "Last viewed chapter not found, using first chapter",
                    );
                }
            } else {
        ...
        try {
            ...
            channel.listen(".project.content.updated", (data: any) => {
                ...
                if (
                    ...
                    if (
                        ["undo_step", "redo_step", "version_switch"].includes(
                            data.trigger,
                        )
                    ) {
                        // Full content replacement for navigation operations
                        console.log(
                            "Updating content from navigation operation:",
                            data.content,
                        );
                        ...
                        console.log(
                            "Switched to parent content for regeneration:",
                            {
                                parentContentLength: parentContent.length,
                            },
                        );
                    } else if (
                        ["llm_generation", "llm_regeneration"].includes(
                            data.trigger,
                        )
                    ) {
                        ...
                        if (!streamCompletionInProgressRef.current) {
                            console.log(
                                "Generation completed (non-streaming path), refreshing navigation info",
                            );
                            setTimeout(() => {
                                router.reload({ only: ["chapters"] });
                            }, 300);
                            ...
                            console.log(
                                "Generation completed but stream completion in progress, skipping reload (onStreamComplete will handle it)",
                            );
                        }
                    }
                }
            ...
            channel.listen(".operation.state.changed", (data: any) => {
                ...
                if (
                    ...
                    console.log(
                        "Updating navigation info directly from websocket:",
                        data.navigationInfo,
                    );
        ...
        return () => {
            if (channel) {
                try {
                    ...
                    console.error(
                        "Error cleaning up content update listener:",
                        error,
                    );
                }
            }
        ...
        (chapterOrder: number) => {
            ...
            if (chapter) {
                ...
                const contentChanged =
                    selectedChapter && content !== originalContent.current;
                const settingsChanged =
                    JSON.stringify(settings) !==
                    JSON.stringify(originalSettings.current);

                if (contentChanged || settingsChanged) {
                    console.log("Saving before chapter switch", {
                        contentChanged,
                        settingsChanged,
                    });
        ...
        (chapterName: string) => {
            if (isGenerating || isStreaming) {
                return Promise.reject(
                    new Error("Cannot add chapters during generation"),
                );
            }
            ...
            return new Promise<void>(async (resolve, reject) => {
                ...
                if (contentChanged || settingsChanged) {
                    console.log("Saving before adding chapter", {
                        contentChanged,
                        settingsChanged,
                    });
    ...
    const initializeChapterHistory = useCallback(
        async (chapterOrder: number, content: string) => {
            try {
                console.log("Initializing chapter history:", {
                    chapterOrder,
    ...
    const handleGenerateText = useCallback(
        (currentEditorContent?: string) => {
            ...
            if (contentChanged || settingsChanged) {
                console.log("Saving before generation", {
                    contentChanged,
                    settingsChanged,
                });
            ...
            safetyTimeoutRef.current = setTimeout(() => {
                console.warn(
                    "Stream safety timeout: no stream completion received within 30s, resetting state",
                );
                setIsGenerating(false);
                setPreservedChapterOrder(null);
                safetyTimeoutRef.current = null;
            ...
            http.post(
                ...
                .catch((error) => {
                    ...
                    console.error(
                        "Failed to start text generation:",
                        normalizedError,
                    );
                    setIsGenerating(false);
                    setPreservedChapterOrder(null);
    ...
    const handleRegenerateText = useCallback(
        (currentEditorContent?: string) => {
            ...
            safetyTimeoutRef.current = setTimeout(() => {
                console.warn(
                    "Regeneration safety timeout: no stream completion received within 30s, resetting state",
                );
                setIsGenerating(false);
                setPreservedChapterOrder(null);
                safetyTimeoutRef.current = null;
            ...
            http.post(
                ...
                .catch((error) => {
                    ...
                    console.error(
                        "Failed to start text regeneration:",
                        normalizedError,
                    );
                    setIsGenerating(false);
                    setPreservedChapterOrder(null);
    ...
    const smartSave = useCallback(
        (order: number, content: string | null, trigger: string) => {
    ...
    return {
        ...
        // Save functions
        saveContent: (order: number, content: string | null, trigger: string) =>
