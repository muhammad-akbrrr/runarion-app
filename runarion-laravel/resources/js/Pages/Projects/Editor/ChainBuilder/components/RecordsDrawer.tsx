...
export const RecordsDrawer: React.FC<RecordsDrawerProps> = ({
    ...
    const [entityTypes, setEntityTypes] = useState<Array<{ value: string; label: string; vertexLabel?: string }>>([
    ...
    const loadCollectionTypes = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const systemTypes = (data.collection_types?.system || []).map((t: any) => ({
                ...
                const customTypes = (data.collection_types?.custom || []).map((t: any) => {
                    // Use vertex_label if available, otherwise normalize the name
                    const typeValue = (t.vertex_label || t.name).toLowerCase().replace(/\s+/g, '_');
                    return {
                        value: typeValue,
                        label: t.name,
    ...
    const filteredEntities = entities.filter((entity) => {
        ...
        if (selectedType !== 'all') {
            ...
            if (normalizedFilter !== normalizedType) {
                ...
                const typeInfo = entityTypes.find(t => t.value === selectedType);
                if (typeInfo) {
                    const typeLabelNormalized = typeInfo.label.toLowerCase().replace(/\s+/g, '_');
    ...
    const getTypeColor = (type: string) => {
        ...
        if (normalized.includes('character')) return 'bg-blue-100 text-blue-700 border-blue-300';
        if (normalized.includes('location')) return 'bg-green-100 text-green-700 border-green-300';
        if (normalized.includes('item')) return 'bg-yellow-100 text-yellow-700 border-yellow-300';
        if (normalized.includes('theme')) return 'bg-purple-100 text-purple-700 border-purple-300';
        if (normalized.includes('plot')) return 'bg-pink-100 text-pink-700 border-pink-300';
    ...
    return (
        <div className="h-full w-64 bg-white border-r border-gray-300 flex flex-col overflow-hidden">
                ...
                <div className="flex gap-1 overflow-x-auto pb-1">
                    {entityTypes.map((type) => (
                        <button
                            ...
                            className={`text-[10px] whitespace-nowrap px-2 py-0.5 rounded-full border transition-colors ${
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {loading ? (
                    <p className="text-xs text-gray-500 text-center mt-10">Loading...</p>
                ) : filteredEntities.length === 0 ? (
                    <p className="text-xs text-gray-500 text-center mt-10">No entities found</p>
                ) : (
