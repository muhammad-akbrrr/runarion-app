...
export const ChainBuilder: React.FC<ChainBuilderProps> = ({
    ...
    const setNodes = useCallback((updater: GraphNode[] | ((prev: GraphNode[]) => GraphNode[])) => {
        setGraphState(prev => ({
            ...prev,
    ...
    const setEdges = useCallback((updater: GraphEdge[] | ((prev: GraphEdge[]) => GraphEdge[])) => {
        setGraphState(prev => ({
            ...prev,
    ...
    const [nodeZOrder, setNodeZOrder] = useState<Map<string, number>>(new Map());
    ...
    const bringNodeToFront = useCallback((nodeId: string) => {
        ...
        setNodeZOrder((prev) => new Map(prev).set(nodeId, zOrderCounterRef.current));
    }, []);
    ...
    }, [undo, redo, selectedNodeIds, selectedEdgeId, deleteSelectedNodes, deleteSelectedEdge]);
    ...
    useEffect(() => {
        if (
            ...
            const currentState: GraphState = JSON.parse(JSON.stringify(graphState));
    ...
    const isAnyLoading = isSavingLocal || isFlowRunning || isBuildingGraph || isRefiningGraph || isWandGenerating;
                ...
                >
                    ...
                    >
                        ...
                        {connectingNodeId && (() => {
                            ...
                            const connectingNodeHeight = nodeHeights.get(connectingNodeId) || 82;
                            const sourceOffset = Math.min(connectingNodeHeight * 0.035, 8);
                            return (
                                <ConnectingLine
                                    sourceX={
                                        (connectingNode?.position.x || 0) + 300
                                    }
                                    sourceY={
                                        (connectingNode?.position.y || 0) + (connectingNodeHeight / 2) + sourceOffset
                                    }
                                    targetX={mousePos.x}
                                    targetY={mousePos.y}
                                    pan={pan}
