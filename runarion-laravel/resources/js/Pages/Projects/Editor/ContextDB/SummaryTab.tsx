...
export default function SummaryTab({ entity, workspaceId, projectId }: SummaryTabProps) {
    ...
    const loadSummaries = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log("Fetch failed with status:", response.status, "- using prop data");
    ...
    const getFilteredSummaries = () => {
        ...
        return summaries.filter(s => s.chapter_number != null && s.chapter_number.toString() === selectedChapter);
    };
        ...
        return (
            <div className="p-4 space-y-4">
                <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold">Chapter Activity Summary</h3>
                    <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                            <p>
                                This tab shows how {entity.name} appears across chapters. 
                                Summaries are generated when you run the Summarizer in the Auditor tab. 
                                Select this entity's category and run summarization to generate chapter-by-chapter activity.
                            </p>
                        </TooltipContent>
                    </Tooltip>
                </div>
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <p className="text-sm text-gray-600">
                        No summaries available yet. Run the Summarizer in the Auditor tab 
                        with category "{entity.type}" selected to generate chapter-by-chapter activity for this entity.
                    </p>
                </div>
            </div>
        );
        ...
        <div className="p-4 space-y-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                            <p>
                                Chapter-by-chapter breakdown of how {entity.name} appears in the story. 
                                Generated by the Summarizer when you analyze this entity's category.
                            </p>
                        </TooltipContent>
                    </Tooltip>
                </div>
                ...
                <Select value={selectedChapter} onValueChange={setSelectedChapter}>
                    ...
                    <SelectContent>
                        <SelectItem value="all">All Chapters ({summaries.length})</SelectItem>
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500">
                <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
                    Appears in {summaries.length} chapter{summaries.length !== 1 ? 's' : ''}
                </span>
                {selectedChapter !== "all" && (
                ...
                {filteredSummaries.map((summary, index) => (
                    summary ? (
                        <div key={index} className="border rounded-lg p-4 bg-white shadow-sm">
                            ...
                            <p className="text-sm text-gray-700 mb-3 leading-relaxed">{summary.activity || 'No activity description available.'}</p>
                            {summary.key_moments && Array.isArray(summary.key_moments) && summary.key_moments.length > 0 && (
                                <div className="mt-3 pt-3 border-t border-gray-100">
                                    <p className="text-xs font-semibold text-gray-600 mb-2">Key Moments:</p>
