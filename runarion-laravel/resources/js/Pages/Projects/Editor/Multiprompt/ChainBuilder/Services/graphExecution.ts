    ...
    for (const nodeId of flatOrder) {
        ...
        try {
            ...
            if (
                ...
                const separator =
                    currentStoryContext && !currentStoryContext.endsWith("\n")
                        ? "\n\n"
                        : "";
                currentStoryContext = currentStoryContext + separator + result;

                console.log(
                    `[Sequence Mode] Appended node ${nodeId} output to story context. New context length: ${currentStoryContext.length}`,
                );
            }
        } catch (error) {
