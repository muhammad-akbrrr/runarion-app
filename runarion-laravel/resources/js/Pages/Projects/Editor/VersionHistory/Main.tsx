...
export default function VersionHistoryPage({
    ...
    const [localChapters, setLocalChapters] = useState<ChapterVersionInfo[]>(chapters);
    ...
    const handleSaveSnapshot = async () => {
        ...
        try {
            ...
            console.log('Saving snapshot:', { url, name: snapshotName, description: snapshotDescription });
            ...
            try {
                ...
                setMessage({ type: 'error', text: `Server error: ${response.status} ${response.statusText}` });
                setIsSaving(false);
                return;
            }
            ...
            if (response.ok && data.success) {
                ...
                setMessage({ type: 'success', text: 'Snapshot saved successfully!' });
                setTimeout(() => setMessage(null), 3000);
            } else {
                console.error('Save failed:', data);
                setMessage({ type: 'error', text: data.error || data.message || 'Failed to save snapshot' });
            }
        } catch (error) {
            console.error('Save snapshot error:', error);
            setMessage({ type: 'error', text: `Failed to save snapshot: ${error instanceof Error ? error.message : 'Unknown error'}` });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleLoadSnapshot = async (snapshotId: string) => {
        if (!confirm('Are you sure you want to load this snapshot? This will restore all chapters to their saved state. You will need to refresh the editor to see the changes.')) {
            return;
        }
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                setTimeout(() => {
                    router.visit(route('workspace.projects.editor', {
                ...
                setMessage({ type: 'error', text: data.error || data.message || 'Failed to load snapshot' });
            }
        } catch (error) {
            console.error('Load snapshot error:', error);
            setMessage({ type: 'error', text: `Failed to load snapshot: ${error instanceof Error ? error.message : 'Unknown error'}` });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDeleteSnapshot = async (snapshotId: string) => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                setLocalSnapshots(localSnapshots.filter(s => s.id !== snapshotId));
                setMessage({ type: 'success', text: 'Snapshot deleted successfully!' });
                setTimeout(() => setMessage(null), 3000);
            } else {
        ...
        return (
            <div className="ml-4 space-y-2">
                {chapter.version_tree.map((node, nodeIndex) => (
                    <div key={node.node_id} className="border-l-2 border-gray-200 pl-4">
    ...
    return (
        ...
        >
            ...
            <div className="w-full h-full flex flex-col bg-gray-50">
                {message && (
                    <Alert className={`m-4 ${message.type === 'success' ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                ...
                <div className="flex-1 flex gap-4 p-4 overflow-hidden min-h-0">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
                            {localSnapshots.length === 0 ? (
                                <div className="text-center text-gray-500 py-8">
                                    ...
                                    <p className="text-sm mt-1">Save your current state to create one</p>
                                </div>
                            ) : (
                    ...
                    <div className="flex-1 flex flex-col bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden min-h-0">
                        <div className="p-4 border-b border-gray-200 flex-shrink-0">
                            <h2 className="text-lg font-semibold">Chapter Versions</h2>
                            <p className="text-sm text-gray-500 mt-1">
                                View and navigate through version history for each chapter
                            </p>
                        </div>
                            ...
                            {localChapters.length === 0 ? (
                                ...
                                <div className="space-y-2">
                                    {localChapters.map((chapter) => (
                                        <Card key={chapter.order} className="hover:shadow-md transition-shadow">
                                            <CardHeader className="pb-3">
                                                <button
                                                ...
                                                >
                                                    ...
                                                    <div className="text-xs text-gray-500">
                                                        {chapter.navigation_info.totalVersions} version{chapter.navigation_info.totalVersions !== 1 ? 's' : ''}
                                                        {' • '}
                                                        Current: v{chapter.navigation_info.currentVersionIndex}
                                                    </div>
                                                </button>
                                            </CardHeader>
                                            {expandedChapters.has(chapter.order) && (
                                                <CardContent>
                                                    {chapter.version_tree.length === 0 ? (
                                                        <p className="text-sm text-gray-500">No version history</p>
                                                    ) : (
                                                        renderVersionTree(chapter)
                                                    )}
                                                </CardContent>
                                            )}
                                        </Card>
            ...
            <Dialog open={isSaveDialogOpen} onOpenChange={setIsSaveDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Create a snapshot of all chapters at their current versions. You can restore this state later.
                        </DialogDescription>
                    </DialogHeader>
