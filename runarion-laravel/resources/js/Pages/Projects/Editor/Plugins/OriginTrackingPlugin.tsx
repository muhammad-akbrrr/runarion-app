    ...
    queueMicrotask(() => {
        editor.update(
            () => {
                ...
                if (!$isOriginTextNode(node) || node.getOrigin() !== "ai")
                    return;
                ...
                if (info.textBefore) {
                    const beforeNode = $createOriginTextNode(
                        info.textBefore,
                        "ai",
                    );
                ...
                const userNode = $createOriginTextNode(
                    info.insertedText,
                    "user",
                );
                ...
                if (info.textAfter) {
                    const afterNode = $createOriginTextNode(
                        info.textAfter,
                        "ai",
                    );
...
export function OriginTrackingPlugin(): null {
    ...
    useEffect(() => {
        ...
        // =================================================================
        const unregisterTextNodeTransform = editor.registerNodeTransform(
            TextNode,
            (node) => {
        ...
        // =================================================================
        const unregisterMergeTransform = editor.registerNodeTransform(
            OriginTextNode,
            (node) => {
        ...
        const unregisterUpdateListener = editor.registerUpdateListener(
            ({ editorState, prevEditorState, tags }) => {
                ...
                editorState.read(() => {
                    ...
                    if (
                        !$isOriginTextNode(anchorNode) ||
                        anchorNode.getOrigin() !== "ai"
                    )
                        return;
                    ...
                    if (!prevNode || !(prevNode instanceof OriginTextNode))
                        return;
                    ...
                    if (currentText.length > prevText.length) {
                        const insertionInfo = detectInsertion(
                            prevText,
                            currentText,
                        );
                        if (insertionInfo) {
        ...
        const unregisterBackspaceCommand = editor.registerCommand(
            ...
            (event) => {
                ...
                if (anchor.offset === 0 && $isOriginTextNode(anchorNode)) {
                    ...
                    if (
                        $isOriginTextNode(prevSibling) &&
                        prevSibling.getOrigin() !== anchorNode.getOrigin()
                    ) {
            ...
            (event) => {
                ...
                if ($isOriginTextNode(anchorNode)) {
                    ...
                    if (anchor.offset === nodeText.length) {
                        ...
                        if (
                            $isOriginTextNode(nextSibling) &&
                            nextSibling.getOrigin() !== anchorNode.getOrigin()
                        ) {
                    ...
                    () => {
                        ...
                        const convertTextNodes = (node: LexicalNode) => {
                            ...
                            if (
                                ...
                                !(node instanceof OriginTextNode)
                            ) {
                                const originNode = $createOriginTextNode(
                                    node.getTextContent(),
                                    "user",
                            ...
                            if (
                                "getChildren" in node &&
                                typeof (node as ElementNode).getChildren ===
                                    "function"
                            ) {
                                (node as ElementNode)
                                    .getChildren()
                        ...
                        if (convertedCount > 0) {
                            console.log(
                                `OriginTrackingPlugin: Converted ${convertedCount} existing TextNodes to OriginTextNodes`,
                            );
                        }
                    },
