...
class InstructionBuilder:
    ...
    def build(self) -> str:
        ...
        if self.config.context:
            parts.append(f"The context of the writing is:\n{self.config.context.strip()}")

        if self.config.genre:
            parts.append(f"The story belongs to the **{self.config.genre.strip()}** genre.")
        ...
        if self.config.pov:
            parts.append(f"The story should follow a **{self.config.pov.strip()}** point of view.")

        if self.config.author_profile:
            parts.append(f"You're expected to write in a style similar to the original author:\n{self.config.author_profile.strip()}")

        parts.append("Please continue the writing professionally, following the guidance above from the given input.")
        return "\n\n".join(parts)
    ...
    def build_from_scratch(self) -> str:
        ...
        if self.config.context:
            parts.append(f"The story should be set in the following context:\n{self.config.context.strip()}")

        if self.config.genre:
            parts.append(f"The genre of the story is **{self.config.genre.strip()}**.")

        if self.config.tone:
            parts.append(f"The tone of the story should be **{self.config.tone.strip()}**.")

        if self.config.pov:
            parts.append(f"The story should be written from a **{self.config.pov.strip()}** point of view.")

        # TODO : Implement lookup to use example of author's writing style
        if self.config.author_profile:
            parts.append(f"Emulate the writing style of the following author profile:\n{self.config.author_profile.strip()}")

        parts.append("Create a compelling story beginning that establishes character, setting, and conflict setting up to a masterpiece story.")
        return "\n\n".join(parts)