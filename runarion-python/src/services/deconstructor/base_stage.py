"""
Base stage interface for the novel deconstruction pipeline.
...
class PipelineStageContext:
    ...
    Enhanced with dynamic values for proper database operations.
    """
    ...
    def __init__(self, draft_id: str, user_id: int = None, workspace_id: str = None, 
        ...
        # Auto-derive missing values if not provided
        self._auto_derive_context()
    
    def _auto_derive_context(self):
        """Auto-derive missing context values from draft data or defaults."""
        if not self.user_id or not self.workspace_id:
            # These will be derived when first accessed if not provided
            self._derived_values_fetched = False
        else:
    ...
    def get_user_id(self, db_pool=None) -> int:
        ...
        Args:
            db_pool: Database pool for deriving user_id if needed
            
        Returns:
            User ID (int) - either provided or derived from draft
        """
        if self.user_id:
            return self.user_id
        ...
        # Fallback to test user if still not available
        return self.user_id or 1  # Test user ID as last resort
    ...
    def get_workspace_id(self, db_pool=None) -> str:
        ...
        Args:
            db_pool: Database pool for deriving workspace_id if needed
            
        Returns:
            Workspace ID (str) - either provided or derived from draft
        """
    ...
    def _derive_context_from_draft(self, db_pool):
        ...
        try:
            ...
            # Silent fallback - will use defaults
            pass
...
class BasePipelineStage(ABC):
    ...
    def run(self, draft_id: str, *args, user_id: int = None, workspace_id: str = None, 
        ...
        """
        Execute the stage with standardized interface and dynamic context.
        
        Args:
            draft_id: UUID of the draft
            *args: Positional arguments (e.g., file_path for Stage 1)
        ...
        """
        # Handle positional arguments by mapping them to context metadata
        context_kwargs = dict(kwargs)
        
        # For Stage 1 (PDFIngestionStage), first positional arg is file_path
        if len(args) > 0 and self.stage_name == "PDFIngestionStage":
            context_kwargs['file_path'] = args[0]
        ...
        try:
            self.logger.info(f"Starting {self.stage_name} for draft {draft_id} (user_id: {context.get_user_id(self.db_pool)})")
    ...
    def run_with_connection(self, conn, draft_id: str, *args, user_id: int = None, 
        ...
        """
        Execute the stage within a database transaction with dynamic context.
        ...
        try:
            self.logger.info(f"Starting {self.stage_name} (transactional) for draft {draft_id} (user_id: {context.get_user_id(self.db_pool)})")
            result = self._execute_stage(context)
            
            if result.success:
                self.logger.info(f"{self.stage_name} completed successfully (transactional) for draft {draft_id}")
            else:
                self.logger.error(f"{self.stage_name} failed (transactional) for draft {draft_id}: {result.data.get('error')}")
                ...
                if result and result[0]:
                    ...
                    if isinstance(result[0], dict):
                        ...
                        try:
                            ...
                            self.logger.warning(f"Invalid JSON in metadata for draft {draft_id}, returning empty dict")
                            return {}
                    else:
                        self.logger.warning(f"Unexpected metadata type {type(result[0])} for draft {draft_id}")
                        return {}
                return {}
    ...
    def get_database_connection(self, context: PipelineStageContext):
        ...
        Returns:
            Database connection or connection manager
        """
        if context.get('connection'):
            # Return the existing connection for transactional operations
    ...
    def validate_required_parameters(self, context: PipelineStageContext, 
                                    required_params: list) -> PipelineStageResult:
        """
        ...
        Returns:
            PipelineStageResult indicating validation success/failure
        """
        missing_params = []
        ...
        return PipelineStageResult.success_result(self.stage_name)
