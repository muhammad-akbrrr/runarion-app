...
class DeconstructorOrchestrator:
    ...
    def _validate_stage_result(self, stage_num: Any, stage_name: str,
        ...
        if stage_num == 1:  # Ingestion
            ...
            boundary_validation = stage_result.get('boundary_validation', {}) or {}
            ...
            if boundary_validation:
                critical_flag_count = int(boundary_validation.get('critical_flag_count', 0) or 0)
                critical_ratio = float(boundary_validation.get('critical_ratio', 0.0) or 0.0)
                flagged_count = int(boundary_validation.get('flagged_count', 0) or 0)

                if critical_flag_count > 0 and critical_ratio > 0.5:
                    error = (
                        "Stage 7 chapter boundary pacing quality too low: "
                        f"{critical_flag_count} critical boundary issue(s), "
                        f"critical_ratio={critical_ratio:.2f}. "
                        "Detected repeated high-kinetic action splits without justification."
                    )
                    logger.error(error)
                    return {
                        'valid': False,
                        'error': error,
                        'validation_type': 'chapter_boundary_quality',
                        'boundary_validation': boundary_validation,
                    }

                if flagged_count > 0:
                    logger.warning(
                        "Stage 7 boundary pacing warnings: "
                        f"{flagged_count} flagged boundary/boundaries "
                        f"(critical={critical_flag_count}, critical_ratio={critical_ratio:.2f})."
                    )
